...
#ifdef WATER_UNDERGROUND
static float4 ssfx_is_underground = float4(1.0, 1.0, 1.0, 1.0);
#else
static float4 ssfx_is_underground = float4(0.0, 0.0, 0.0, 0.0);
#endif
...
void SSFX_ScreenSpaceReflections(float2 tc, float4 P, float3 N, float gloss, inout float3 color, uint iSample : SV_SAMPLEINDEX)
{
	...
	// Attenuation
	float attenuation = smoothstep(rain_params.z/2.f, rain_params.w/2.f, length( P.xyz ));
	refl_power *= 1.0f - attenuation;
