...
//#define ssfx_pixel_size float2(1.0f / screen_res.xy);
...
{
	#ifndef USE_MSAA
		float4 rL = s_accum_diffuse.Sample(smp_nofilter, tc);
		float4 rS = s_accum_spec.Sample(smp_nofilter, tc);
		float4 C = s_diffuse.Sample( smp_nofilter, tc );
	#else
		float4 rL = s_accum_diffuse.Load(int3(tc * screen_res.xy, 0), iSample);
		float4 rS = s_accum_spec.Load(int3(tc * screen_res.xy, 0), iSample);
...
float3 SSFX_get_scene(float2 tc, uint iSample : SV_SAMPLEINDEX)
{
	...
	if (length(rL) > 10.0f || length(rS) > 10.0f)
		rL = 0;
	...
	float3 nw = mul( m_inv_V, rN );
		...
		rL.rgb += rS.rgb * SRGBToLinear(rD.rgb);
		...
		// Final color
		float4 light = float4(rL.rgb + hdiffuse, 1.f);
		float4 C = rD * light;
		float3 spec = C.www * rS.rgb;
