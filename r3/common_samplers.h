...
Texture2DMS<float4>                 s_normal;	//	smp_nofilter or Load
#else
...
#ifdef USE_MSAA
#ifdef MSAA_OPTIMIZATION
Texture2DMS<float4>                 s_diffuse;	// rgb.a = diffuse.specular
Texture2DMS<float4>                 s_accum_diffuse;      	// rgb = diffuse
Texture2DMS<float4>                 s_accum_spec;      	// rgb = specular
#else
Texture2DMS<float4, MSAA_SAMPLES>	s_diffuse;	// rgb.a = diffuse.specular
Texture2DMS<float4, MSAA_SAMPLES>       s_accum_diffuse;      	// rgb = diffuse
Texture2DMS<float4, MSAA_SAMPLES>       s_accum_spec;      	// rgb = specular
#endif
#else
Texture2D	s_diffuse;	// rgb.a = diffuse.specular
Texture2D	s_accum_diffuse;      	// rgb = diffuse
Texture2D	s_accum_spec;      	// rgb = specular
#endif
